...
# ── Fragment — only this part reruns on click ──
@st.fragment
def calculator():
    # Display
    st.markdown(f"""
    ...
    # Button colors via CSS per column index
    btn_styles = {
        "sp": "#636366",
        "nm": "#3a3a3c",
        "op": "#FF9F0A",
    ...
    for i, (label, style) in enumerate(buttons):
        ...
        color_css += f"""
        div[data-testid="column"]:nth-child({(i % 4) + 1}) div.stButton > button {{
